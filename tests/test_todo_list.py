...
class TestToDoList(BaseClass, BaseClass.ToDoList): 
     ...
     def test_2_mark_item_as_complete(self):
          ...
          self.click_checkbox(selected_id)                    # Check checkbox 
     ...
     def test_3_mark_item_as_active(self):
          ...
          self.click_checkbox(selected_id)                       # Select checkbox 
     ...
     def test_6_delete_todos(self):
          selected_id = self.list_of_all_ids_current_tab()[1]         # select the ID to be deleted. Will be used to build the selector and to check if the right ID wa deleted.
          self.delete_selected_id(selected_id)                        # hover over the selected item
         
          self.assert_delete_id_not_in_active_tab(selected_id)      # Ensure deleted ID is not present on ACTIVE TAB.
          self.go_to_all_tab()                                                                
          self.assert_delete_id_not_in_all_tab(selected_id)  # Ensure deleted ID is not present on ALL TAB.
