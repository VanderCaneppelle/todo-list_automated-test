...
class TestToDoList(BaseClass, BaseClass.ToDoList): # testsuite
     ...
     def test_2_mark_item_as_completed_and_test_left_items(self):
          ...
          active_ids_from_all_tab = self.list_of_active_ids_current_tab(TODO_COMPLETED_IDS,TODO_LIST_ITEMS)   # select all the active IDS from ALL tab
     ...
     def test_3_completed_tab(self):
          ...
          all_ids_from_completed_tab = self.list_of_all_ids_current_tab(TODO_LIST_ITEMS)  # get all ids present on COMPLETED tab.
     ...
     def test_4_active_tab(self):
          ...
          active_ids_from_all_tab = self.list_of_active_ids_current_tab(TODO_COMPLETED_IDS,TODO_LIST_ITEMS) # Get active items frmo ALL tab, to compare with active item from ACTIVE tab.
          self.get_element(ACTIVE_BTN).click()                                                              # Click on ACTIVE
          active_list = self.list_of_all_ids_current_tab(TODO_LIST_ITEMS)                                   # Get list of all ids from ACTIVE tab
     ...
     def test_5_delete_todo(self):
          ...
          assert id_selection  not in self.list_of_active_ids_current_tab(TODO_COMPLETED_IDS,TODO_LIST_ITEMS)      # Ensure deleted ID is not present on ACTIVE tab.
          self.get_element(ALL_BTN).click()                                                                      
          assert id_selection not in self.list_of_active_ids_current_tab(TODO_COMPLETED_IDS,TODO_LIST_ITEMS)       # Ensure deleted ID is not present on ALL tab.
     ...
     def test_6_clear_completed_btn(self):
          ...
          all_tab_completed_id = self.list_of_completed_ids_current_tab(TODO_COMPLETED_IDS)    # Get all the completed ID from ALL tab
          self.get_element(COMPLETE_BTN).click()                                               # Go to COMPLETE tab
          completed_tab_all_ids = self.list_of_all_ids_current_tab(TODO_LIST_ITEMS)            # Get all IDS from COMPLETE tab
